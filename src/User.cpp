...
User::User(const User &value)
    : username(value.username), realname(value.realname), nickname(value.nickname), host(value.host) {}
...
bool User::operator==(const User &value) const {
  return username == value.username && realname == value.realname && nickname == value.nickname;
};
